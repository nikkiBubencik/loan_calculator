...
def get_loans():
  ...
  num_loans = input("How many loans do you have ")
...
def sort_loans_by_interest_rate(loan_dict):
    ...
    sorted_loans = sorted(loan_dict.values(), key=lambda item: (item.interest_rate, item.principal_float), reverse=True)
...
def avalanche(loan_dict):
  ...
  while len(active_loans_list) > 0:
    sub_payment:float = payment_amount
    for i in range(len(active_loans_list)):
      sub_payment = active_loans_list[i].make_payment(sub_payment)
      if active_loans_list[i].get_total_left() == 0:
        paid_off_loans_list.append((active_loans_list[i], current_date))
